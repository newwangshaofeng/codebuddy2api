"""
CodeBuddy V2 API Router - 兼容CodeBuddy官方API格式
"""
import json
import time
import uuid
...
async def chat_completions(
    ...
    try:
        ...
        # 生成请求头
        headers = generate_codebuddy_headers(
            ...
            if all_chunks:
                ...
                if "choices" in base_response and base_response["choices"]:
                    ...
                    if "delta" in choice:
                        ...
                        # 合并所有块的内容（包括第一个块）
                        merged_content = merged_delta.get("content", "")
        ...
        logger.error(f"CodeBuddy V2 API错误: {e}")
        raise HTTPException(status_code=500, detail=f"内部服务器错误: {str(e)}")

@router.get("/v2/models")
async def list_v2_models(_token: str = Depends(authenticate)):
    """获取CodeBuddy V2模型列表"""
    try:
        # 获取凭证
        credential = codebuddy_token_manager.get_next_credential()
